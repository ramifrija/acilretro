...
import { Search, Plus, Minus, Trash2, ShoppingCart, X, Check, Barcode, ChevronDown, Percent, Receipt, User, Car, FileText } from 'lucide-react';
...
export default function AdminPOS() {
  ...
  return (
    <>
      <div className={`animate-fade-in h-full ${printDoc ? 'hidden' : ''} print:hidden`}>
                  ...
                  {paginated.map((p) => {
                    ...
                    return (
                      <button
                        ...
                        className={`bg-white dark:bg-brand-900 rounded-2xl border border-slate-200 dark:border-white/10 text-left transition-all duration-200 group relative overflow-hidden flex flex-col
                        ${isOutOfStock ? 'opacity-50 cursor-not-allowed grayscale-[0.5]' : 'hover:border-brand-500 hover:shadow-lg hover:-translate-y-1 active:scale-[0.98] cursor-pointer'}
                      `}
                      >
                        {/* Image Section */}
                        <div className="aspect-[4/3] w-full bg-slate-50 dark:bg-white/5 relative p-4 flex items-center justify-center shrink-0 border-b border-slate-100 dark:border-white/5">
                          {p.images?.[0] ? (
                          ...
                          <div className="absolute top-2 left-2 flex flex-col gap-1 z-10">
                            {hasOptions && (
                              <span className="bg-slate-800 text-white text-[9px] font-bold px-2 py-0.5 rounded shadow-sm tracking-wide">
                                OPTIONS
                              </span>
                            )}
                            {p.promo_price && (
      ...
      {selectedProduct && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 animate-fade-in">
          ...
          <div className="relative bg-white dark:bg-brand-950 rounded-3xl w-full max-w-lg p-6 md:p-8 shadow-2xl">
            <button onClick={() => setSelectedProduct(null)} className="absolute top-4 right-4 w-10 h-10 flex items-center justify-center rounded-full bg-slate-100 dark:bg-white/10 text-slate-500 hover:bg-slate-200 dark:hover:bg-white/20 transition-all z-10">
              <X className="w-5 h-5" />
            </button>
              ...
              {selectedProduct.product_options?.map((opt) => (
                <div key={opt.id}>
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {opt.required && (
                      <span className="text-[10px] uppercase font-bold px-2 py-0.5 rounded-md bg-error-50 text-error-600 border border-error-100">
                        Requis
                      </span>
                    )}
                  </div>
                    ...
                    {opt.option_values.map((val) => {
                      ...
                      return (
                        <button
                          ...
                          className={`p-3 text-left rounded-2xl border-2 transition-all duration-200 flex flex-col ${isSelected
                            ? 'border-brand-500 bg-brand-50 dark:bg-brand-900/40 ring-4 ring-brand-500/10'
                            : 'border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-brand-900/10 hover:border-brand-300 hover:bg-white'
                            }`}
                        >
                          {val.image_url && (
                            <div className="w-full h-20 bg-white dark:bg-brand-950 rounded-xl mb-3 p-2 flex items-center justify-center shrink-0 shadow-sm border border-slate-100 dark:border-white/5">
                              <img src={val.image_url} alt={val.value} className="w-full h-full object-contain" />
                            </div>
                          )}
                          <div className="flex justify-between items-center w-full gap-2 mt-auto">
                            <span className={`text-[13px] font-bold leading-tight ${isSelected ? 'text-brand-700 dark:text-brand-300' : 'text-slate-700 dark:text-slate-300'}`}>
                              {val.value}
                            </span>
                            {val.price_modifier > 0 && (
                              <span className="text-[11px] font-black text-success-600 bg-success-50 px-1.5 py-0.5 rounded shrink-0">
            ...
            <div className="mt-8 pt-6 border-t border-slate-100 dark:border-white/10">
              <button
                ...
                className="w-full py-4 rounded-2xl bg-brand-600 hover:bg-brand-700 text-white font-bold text-lg shadow-xl shadow-brand-500/30 disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2"
              >
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 animate-fade-in">
          ...
          <div className="relative glass-card w-full max-w-md p-8 text-center shadow-2xl">
            ...
            <h2 className="font-display font-bold text-2xl text-slate-900 dark:text-white mb-1">
              {isDeliveryMode ? 'Bon de Livraison' : 'Confirmation'}
            </h2>
            <p className="text-slate-500 text-sm mb-6">
              {isDeliveryMode ? 'Valider et imprimer le bon de livraison' : "Valider l'encaissement et imprimer la facture"}
            </p>
            ...
            <p className="text-xs text-slate-400 mb-5">
              {isDeliveryMode ? "Le bon de livraison s'ouvrira automatiquement après la validation" : "La facture s'ouvrira automatiquement après l'encaissement"}
            </p>
