        ...
        <div className="flex flex-col gap-4 min-h-0">
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="Rechercher par nom, SKU ou r├®f├®rence OEM..."
          ...
          {/* Brand Filter ÔÇö Searchable Dropdown */}
          ...
          <div className="flex-1 overflow-y-auto pb-4">
            {filtered.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 text-slate-400">
                ...
                <p className="text-base font-medium">Aucun produit trouv├®</p>
                      ...
                      <div className="aspect-square rounded-xl mb-3 overflow-hidden bg-slate-100 dark:bg-brand-900/30 relative">
                        ...
                        {isOutOfStock && (
                          <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
                            <span className="text-white text-xs font-bold bg-black/60 px-2 py-1 rounded-lg">├ëpuis├®</span>
                          </div>
                        )}
                      </div>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between mt-5 pt-4 border-t border-slate-100 dark:border-white/10">
                <span className="text-xs text-slate-500">
                  {((page - 1) * ITEMS_PER_PAGE) + 1}ÔÇô{Math.min(page * ITEMS_PER_PAGE, filtered.length)} sur {filtered.length} produits
                </span>
                <div className="flex items-center gap-1.5">
                  ...
                  {Array.from({ length: totalPages }, (_, i) => i + 1)
                    ...
                    .map((n, i) =>
                      n === '...' ? (
                        <span key={`ellipsis-${i}`} className="px-1 text-slate-400 text-xs">ÔÇª</span>
                      ) : (
                        <button
                          key={n}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {item.options.length > 0 && (
                        <div className="text-[10px] text-brand-500 dark:text-brand-400 mt-0.5 truncate">
                          {item.options.map(o => o.value).join(' ┬À ')}
                        </div>
                      )}
                      {/* Qty Controls */}
                      <div className="flex items-center gap-1.5 mt-2">
                        ...
                        <span className="text-[10px] text-slate-400 ml-1">├ù {formatPrice(item.unitPrice)}</span>
                      </div>
                    </div>
                    ...
                    {clients
                      ...
                      .map(c => (
                        ...
                        >
                          ...
                          {(c.num_tel || c.email) && (
                            <div className="text-xs text-slate-500">{c.num_tel} {c.email ? `ÔÇó ${c.email}` : ''}</div>
                          )}
                        </button>
                      ))}
                  </div>
      ...
      {showCheckout && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 animate-fade-in">
          ...
          <div className="relative glass-card w-full max-w-md p-8 text-center shadow-2xl">
            ...
            <p className="text-xs text-slate-400 mb-5">La facture s'ouvrira automatiquement apr├¿s l'encaissement</p>
